...
# Fixed chart canvas + dedicated legend rows.
def _render_chart_with_state(fig, desc_index):
    ...
    fig.update_layout(
        template="plotly_white", width=None, height=400,
        margin=dict(l=72, r=72, t=150, b=42),
        ...
        legend2=dict(orientation="h", yanchor="bottom", y=1.105, xanchor="left", x=0.02, xref="paper", font=dict(size=10), bgcolor="rgba(255,255,255,0)", traceorder="normal", entrywidthmode="pixels", entrywidth=145),
        legend3=dict(orientation="h", yanchor="bottom", y=1.01, xanchor="left", x=0.02, xref="paper", font=dict(size=10), bgcolor="rgba(255,255,255,0)", traceorder="normal", entrywidthmode="pixels", entrywidth=145),
    )
    payload = json.dumps(pio.to_json(fig, validate=False, pretty=False), ensure_ascii=False)
    html = f"""
<!doctype html><html><head><script src="https://cdn.plot.ly/plotly-2.35.2.min.js"></script><style>html,body,#chart{{margin:0;padding:0;width:100%;height:100%;overflow:hidden;background:transparent;}}</style></head><body><div id="chart"></div><script>
const chartId={json.dumps(chart_id)}; const storageKey="macro-dashboard-legend:"+chartId; const fig=JSON.parse({payload}); const gd=document.getElementById("chart");
...
Plotly.newPlot(gd,fig.data||[],fig.layout||{{}},{{displayModeBar:false,scrollZoom:false,doubleClick:false,editable:false,displaylogo:false,responsive:true}}).then(()=>{{restoreHidden();gd.on("plotly_legendclick",ev=>{{const i=ev.curveNumber,t=gd.data[i],k=keyOf(t,i),hidden=readHidden();if(t.visible==="legendonly"){{Plotly.restyle(gd,{{visible:true}},[i]);hidden.delete(k);}}else{{Plotly.restyle(gd,{{visible:"legendonly"}},[i]);hidden.add(k);}}writeHidden(hidden);return false;}});gd.on("plotly_legenddoubleclick",()=>{{setTimeout(()=>{{const hidden=readHidden();(gd.data||[]).forEach((t,i)=>{{const k=keyOf(t,i);if(t.visible==="legendonly")hidden.add(k);else hidden.delete(k);}});writeHidden(hidden);}},50);return true;}});}});
</script></body></html>
"""
...
def _apply_dual_y_layout(fig, left_title, right_title):
    # Keep both axes on the same plot edge, with the right axis anchored to x.
    # This avoids the previous free-position drift and keeps tick labels aligned.
    fig.update_layout(
        yaxis=dict(title=dict(text=left_title, standoff=8), side="left", anchor="x", fixedrange=True, automargin=True, ticks="outside", ticklen=4, tickwidth=1, showgrid=True, gridcolor="#eeeeee", zeroline=False),
        yaxis2=dict(title=dict(text=right_title, standoff=8), overlaying="y", side="right", anchor="x", fixedrange=True, automargin=True, ticks="outside", ticklen=4, tickwidth=1, showgrid=False, zeroline=False, tickmode="sync"),
    )
...
def build_fig1(date_range):
    ...
    bad_iorb = (data["observation_date"] >= pd.Timestamp("2021-07-29")) & (data["IORB"] <= 0); data.loc[bad_iorb, "IORB"] = pd.NA
    data = filter_range(data, date_range); data["SOFR_minus_IORB_bp"] = (data["SOFR"] - data["IORB"]) * 100.0
    fig = go.Figure()
    ...
    add_line(fig, data, "SOFR_minus_IORB_bp", "SOFR−IORB", 2.2, "dot", "y2", " bp"); _apply_split_legends(fig, ["SOFR−IORB"], ["ON RRP"])
    _apply_dual_y_layout(fig, "Rate (%)", "Spread (bp)"); fig.update_traces(selector=dict(name="SOFR−IORB"), hovertemplate="SOFR−IORB: %{y:.1f} bp<extra></extra>")
...
def build_fig3(date_range):
    ...
    add_line(fig, data, "T10Y2Y_bp", "10Y−2Y", 2.2, "dot", "y2", " bp"); add_line(fig, data, "T10Y3M_bp", "10Y−3M", 2.2, "dash", "y2", " bp"); _apply_split_legends(fig, ["10Y−2Y", "10Y−3M"], ["2Y"])
    ...
    _apply_dual_y_layout(fig, "Yield (%)", "Spread (bp)")
    return apply_chart_style(fig, chart_height(400, 400))

def build_fig4(date_range):
    reserve = _clean_chart_frame(get_wresbal(), "WRESBAL"); tga = _clean_chart_frame(get_wtre_gen(), "WTREGEN"); rrp = _clean_chart_frame(get_rrp_daily(), "RRPONTSYD")
    reserve["WRESBAL"] /= 1_000_000.0; tga["WTREGEN"] /= 1_000_000.0; rrp["RRPONTSYD"] /= 1_000.0
...
source = source.replace('[("IORB (IORB)", "https://fred.stlouisfed.org/series/IORB"), ("ON RRP Rate (RRPONTSYAWARD)", "https://fred.stlouisfed.org/series/RRPONTSYAWARD"), ("EFFR (EFFR)", "https://fred.stlouisfed.org/series/EFFR"), ("SOFR (SOFR)", "https://fred.stlouisfed.org/series/SOFR")]', '[("IORB (IORB)", "https://fred.stlouisfed.org/series/IORB"), ("ON RRP Rate (RRPONTSYAWARD)", "https://fred.stlouisfed.org/series/RRPONTSYAWARD"), ("EFFR (EFFR)", "https://fred.stlouisfed.org/series/EFFR"), ("SOFR (SOFR)", "https://fred.stlouisfed.org/series/SOFR"), ("Inputs: SOFR + IORB → Formula: (SOFR − IORB) × 100 bp", "")]')
source = source.replace('[("3M Treasury (DGS3MO)", "https://fred.stlouisfed.org/series/DGS3MO"), ("2Y Treasury (DGS2)", "https://fred.stlouisfed.org/series/DGS2"), ("10Y Nominal (DGS10)", "https://fred.stlouisfed.org/series/DGS10"), ("10Y−2Y Spread (T10Y2Y)", "https://fred.stlouisfed.org/series/T10Y2Y"), ("10Y−3M Spread (T10Y3M)", "https://fred.stlouisfed.org/series/T10Y3M")]', '[("3M Treasury (DGS3MO)", "https://fred.stlouisfed.org/series/DGS3MO"), ("2Y Treasury (DGS2)", "https://fred.stlouisfed.org/series/DGS2"), ("10Y Nominal (DGS10)", "https://fred.stlouisfed.org/series/DGS10"), ("10Y−2Y Spread (T10Y2Y)", "https://fred.stlouisfed.org/series/T10Y2Y"), ("10Y−3M Spread (T10Y3M)", "https://fred.stlouisfed.org/series/T10Y3M"), ("Derived display: T10Y2Y/T10Y3M × 100 = bp", "")]')
source = source.replace('[("Reserve Balances (WRESBAL)", "https://fred.stlouisfed.org/series/WRESBAL"), ("TGA (WTREGEN)", "https://fred.stlouisfed.org/series/WTREGEN"), ("ON RRP Balance (RRPONTSYD)", "https://fred.stlouisfed.org/series/RRPONTSYD")]', '[("Reserve Balances (WRESBAL)", "https://fred.stlouisfed.org/series/WRESBAL"), ("TGA (WTREGEN)", "https://fred.stlouisfed.org/series/WTREGEN"), ("ON RRP Balance (RRPONTSYD)", "https://fred.stlouisfed.org/series/RRPONTSYD"), ("Inputs: WRESBAL + WTREGEN + RRPONTSYD → Formula: WRESBAL − WTREGEN − RRPONTSYD", "")]')
source = source.replace('def add_sources(sources):\n    links = [f\'<a href="{html.escape(url, quote=True)}" target="_blank" rel="noopener noreferrer">{html.escape(text)}</a>\' for text, url in sources]\n    st.markdown(\'<div class="source-text">Source: \' + \'<span class="source-sep">|</span>\'.join(links) + \'</div>\', unsafe_allow_html=True)', 'def add_sources(sources):\n    parts = []\n    for text, url in sources:\n        if url: parts.append(f\'<a href="{html.escape(url, quote=True)}" target="_blank" rel="noopener noreferrer">{html.escape(text)}</a>\')\n        else: parts.append(html.escape(text))\n    st.markdown(\'<div class="source-text">Source: \' + \'<span class="source-sep">|</span>\'.join(parts) + \'</div>\', unsafe_allow_html=True)')
