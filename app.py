...
def _finalize_chart(fig, right_names=(), right_title=None, left_title=None, date_range=None):
    ...
    fig.update_layout(height=400, margin=dict(l=58, r=58, t=150, b=62, pad=0, autoexpand=False),
    ...
    fig.update_yaxes(automargin=False, fixedrange=True, ticks="outside", ticklen=4, tickwidth=1, tickfont=dict(size=11), nticks=8, showgrid=True, gridcolor="#eeeeee")
    if left_title is not None:
        fig.update_layout(yaxis=dict(title=left_title, side="left", anchor="x", fixedrange=True, automargin=False, ticks="outside", ticklabelposition="outside", tickformat=".2f", nticks=8, tickfont=dict(size=11)))
    if right_names or any(getattr(t, "yaxis", "y") == "y2" for t in fig.data):
        fig.update_layout(yaxis2=dict(title=right_title or "Spread (bp)", overlaying="y", side="right", anchor="x", fixedrange=True, automargin=False, ticks="outside", ticklen=4, tickwidth=1, tickfont=dict(size=11), nticks=8, ticklabelposition="outside", showgrid=False, zeroline=True, zerolinecolor="#9ca3af", tickmode="sync"))
