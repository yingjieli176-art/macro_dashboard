...
Plotly.newPlot(gd,fig.data||[],fig.layout||{{}},{{displayModeBar:false,scrollZoom:false,doubleClick:false,editable:false,displaylogo:false,responsive:true}}).then(()=>{{
  ...
  gd.on("plotly_legendclick",ev=>{{
    ...
    if(t.visible==="legendonly"){{Plotly.restyle(gd,{{visible:true}},[i]);hidden.delete(k);}}
    else{{Plotly.restyle(gd,{{visible:"legendonly"}},[i]);hidden.add(k);}}
    writeHidden(hidden); return false;
  }});
...
# -------------------------------------------------------------------------
# All four charts use official FRED series.  Do not synthesize an official
# series when FRED already publishes it.  Only unit conversion and the clearly
# labeled Dashboard-derived SOFR−IORB / Net Liquidity calculations remain.
chart_override = r'''
...
def build_fig1(date_range):
    ...
    # IORB has been a positive administered rate since its introduction.
    # Never display an artificial 0 caused by malformed/missing source data.
    ...
    fig.update_layout(
        ...
        yaxis2=dict(title="Spread (bp)", overlaying="y", side="right", anchor="free", position=1.0,
...
def build_fig2(date_range):
    frames = [
        _clean_chart_frame(get_dgs10(), "DGS10"),
        _clean_chart_frame(get_dfii10(), "DFII10"),
        _clean_chart_frame(get_fred_series("T10YIE"), "T10YIE"),
    ]
    data = frames[0]
    for frame in frames[1:]:
        data = data.merge(frame, on="observation_date", how="outer")
...
def build_fig3(date_range):
    frames = [
        _clean_chart_frame(get_dgs3mo(), "DGS3MO"),
        _clean_chart_frame(get_dgs2(), "DGS2"),
        _clean_chart_frame(get_dgs10(), "DGS10"),
        _clean_chart_frame(get_fred_series("T10Y2Y"), "T10Y2Y"),
        _clean_chart_frame(get_fred_series("T10Y3M"), "T10Y3M"),
    ]
    data = frames[0]
    for frame in frames[1:]:
        data = data.merge(frame, on="observation_date", how="outer")
    ...
    # T10Y2Y and T10Y3M are already official FRED spread series in percent;
    # only convert percent -> basis points for display.
...
def build_fig4(date_range):
    # WRESBAL and WTREGEN are official H.4.1 weekly averages, ending Wednesday,
    # in millions of USD. RRPONTSYD is official NY Fed daily ON RRP balance,
    # in billions of USD. Keep RRP daily; forward-fill only the weekly series.
    reserve = _clean_chart_frame(get_wresbal(), "WRESBAL")
    tga = _clean_chart_frame(get_wtre_gen(), "WTREGEN")
    rrp = _clean_chart_frame(get_rrp_daily(), "RRPONTSYD")
    ...
    # Dashboard-derived indicator only. No official FRED series with this
    # exact definition is substituted for it.
...
# Inject the audited chart builders immediately before the original chart render call.
marker = "render_core_charts()"
if marker not in source:
    raise RuntimeError("render_core_charts marker not found")
source = source.replace(marker, chart_override + "\n" + marker, 1)

# Keep UI labels consistent with the audited chart content.
