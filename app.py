...
def render_core_charts():
    ...
    if compact_mode:
        ...
        st.markdown('<div class="compact-title">5. Hong Kong Liquidity</div>', unsafe_allow_html=True); st.markdown('<div class="compact-description">HKD M2/M3 MoM / Monetary Base MoM / Aggregate Balance / HIBOR / O/N−3M Spread (R) / USD-HKD / 7.75-7.85 CU</div>', unsafe_allow_html=True); hk_range = st.radio("时间范围", RANGES, horizontal=True, index=1, key="compact_hk_liquidity_range", label_visibility="collapsed"); st.plotly_chart(build_fig5(hk_range), use_container_width=True, config=PLOTLY_CONFIG); st.markdown(liquidity_status_html(), unsafe_allow_html=True); add_sources([("HKMA Monetary Statistics", "https://apidocs.hkma.gov.hk/documentation/market-data-and-statistics/monthly-statistical-bulletin/financial/monetary-statistics/"), ("HKMA Open API", "https://apidocs.hkma.gov.hk/")])
    else:
        ...
        st.markdown('<div class="section-title">5. Hong Kong Liquidity</div>', unsafe_allow_html=True); st.markdown('<div class="section-description">HKD M2/M3 月环比、货币基础月环比、银行体系总结余、HIBOR、O/N−3M 利差（R）与 USD/HKD 强弱方兑换保证</div>', unsafe_allow_html=True); hk_range = st.radio("时间范围", RANGES, horizontal=True, index=1, key="normal_hk_liquidity_range", label_visibility="collapsed"); st.plotly_chart(build_fig5(hk_range), use_container_width=True, config=PLOTLY_CONFIG); st.markdown(liquidity_status_html(), unsafe_allow_html=True); st.markdown('<div class="mini-description"><b>参数概念：</b><br>1. HKD M2 MoM：港元 M2 月环比增速，用来观察广义港元货币的边际扩张或收缩；比同比变化更适合判断当前流动性方向。<br>2. HKD M3 MoM：港元 M3 月环比增速，统计口径较 M2 更广，用于交叉确认广义货币边际变化。<br>3. Monetary Base MoM：香港货币基础总量月环比变化，用于观察基础货币层面的边际扩张与收缩。<br>4. Aggregate Balance：银行体系总结余，单位由 HK$ million 转为 HK$ billion；总结余下降通常代表银行体系可用港元流动性趋紧。<br>5. O/N HIBOR：隔夜港元银行同业拆息，反映最短端港元资金价格。<br>6. 3M HIBOR：3 个月港元银行同业拆息，用来观察更持续的港元融资成本。<br>7. HKMA Base Rate：香港金管局基本利率，是港元利率体系的重要政策参考。<br>8. O/N−3M Spread（R）：隔夜 HIBOR 减 3M HIBOR，右轴单位 bp；显著转正通常代表短端资金压力上升。<br>9. USD/HKD：每 1 美元对应的港元价格；向 7.85 上升表示港元转弱，向 7.75 下降表示港元转强。<br>10. Strong-side CU 7.75：联系汇率制度下强方兑换保证。<br>11. Weak-side CU 7.85：联系汇率制度下弱方兑换保证。<br><br><b>读取提示：</b>M2/M3 为月度统计，公布存在时滞；最新月份如果尚未公布不会向前填充。主图使用 MoM 观察边际变化，流动性评分使用最近 3 个月 M2/M3 MoM 均值降低单月噪声。图表时间范围以 HKMA 最新已发布月份为基准，避免 1M/3M 因发布时间滞后被错误过滤为空。</div>', unsafe_allow_html=True); add_sources([("HKMA Monetary Statistics", "https://apidocs.hkma.gov.hk/documentation/market-data-and-statistics/monthly-statistical-bulletin/financial/monetary-statistics/")])
