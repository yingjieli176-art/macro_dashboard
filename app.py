...
def _get_eastmoney_quote_safe(symbol):
    ...
    if not secid:
        return _empty_quote()
    try:
        response = requests.get(EASTMONEY_QUOTE_URL, params={"secid": secid, "fields": "f43,f57,f58,f169,f170,f46,f44,f45,f47,f48,f60,f86", "ut": EASTMONEY_UT, "fltt": "2", "invt": "2"}, headers={"User-Agent": "Mozilla/5.0", "Referer": "https://quote.eastmoney.com/"}, timeout=8)
        response.raise_for_status()
...
def _confirm_search(key, market):
    ...
    if results:
        ...
        selected = {"symbol": _direct_symbol(market, query), "name": query, "exchange": "", "market": market}
