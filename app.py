...
# Keep every chart on the same fixed plotting geometry. Plotly's automatic
# axis-margin expansion otherwise makes dual-axis charts narrower/different.
...
def _apply_split_legends(fig, right_names=(), second_row_names=()):
    ...
    for trace in fig.data:
        if getattr(trace, "yaxis", "y") == "y2" or getattr(trace, "name", None) in right_names:
            trace.legend = "legend2"
        else:
            trace.legend = "legend"
    for name in second_row_names:
        for trace in fig.data:
            if getattr(trace, "name", None) == name and getattr(trace, "yaxis", "y") != "y2":
                trace.legend = "legend3"

def _apply_secondary_yaxis(fig, title="Spread (bp)"):
    fig.update_layout(
        yaxis=dict(side="left", anchor="x", fixedrange=True, automargin=False, ticks="outside", ticklabelposition="outside"),
        yaxis2=dict(title=title, overlaying="y", side="right", anchor="x", fixedrange=True, automargin=False, ticks="outside", ticklabelposition="outside", showgrid=False, zeroline=True, zerolinecolor="#9ca3af", tickmode="sync"),
    )
...
def build_fig1(date_range):
    ...
    _apply_split_legends(fig, [], ["ON RRP"])
    _apply_secondary_yaxis(fig)
...
def build_fig4(date_range):
    ...
    reserve["WRESBAL"] /= 1_000_000.0
    tga["WTREGEN"] /= 1_000_000.0
    rrp["RRPONTSYD"] /= 1_000.0
