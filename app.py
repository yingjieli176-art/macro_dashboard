...
def _empty_quote():
    return {
        "price": None, "change_pct": None, "market_state": "", "currency": "",
        "post_price": None, "post_change_pct": None, "pre_price": None, "pre_change_pct": None,
        "regular_market_time": None, "post_market_time": None, "pre_market_time": None,
        "quote_source": "", "delayed_by": None, "data_source": "",
    }
...
def _get_eastmoney_quote_safe(symbol):
    ...
    try:
        ...
        price = None if price_raw in (None, "-") else float(price_raw) / 100
        previous = None if prev_raw in (None, "-") else float(prev_raw) / 100
        ...
        state = "REGULAR" if price is not None else "CLOSED"
        return {
            "price": price,
...
def _quote_meta(row, market=""):
    ...
    if delayed not in (None, 0, "0") and source == "Yahoo Finance":
        ...
        parts.append(source)
    if market == "US" and row.get("market_state") not in ("REGULAR", "PRE") and row.get("post_price") is not None:
        parts.append(f"盘后 {row['post_price']:,.2f}")
    return " · ".join(parts)
...
def render_market_groups():
    quotes = {
        "nasdaq": _get_cached_quote("^IXIC"), "sp500": _get_cached_quote("^GSPC"), "dow": _get_cached_quote("^DJI"),
        "hsi": _get_cached_quote("^HSI"), "hstech": _get_cached_quote("HSTECH.HK"),
        "sh": _get_cached_quote("000001.SS"), "sz": _get_cached_quote("399001.SZ"), "csi300": _get_cached_quote("000300.SS"),
    }
    groups = [
        ("🇺🇸 美股", [_market_item_html("纳斯达克", quotes["nasdaq"].get("price"), quotes["nasdaq"].get("change_pct"), _quote_meta(quotes["nasdaq"], "US")), _market_item_html("标普500", quotes["sp500"].get("price"), quotes["sp500"].get("change_pct"), _quote_meta(quotes["sp500"], "US")), _market_item_html("道琼斯", quotes["dow"].get("price"), quotes["dow"].get("change_pct"), _quote_meta(quotes["dow"], "US"))], "three"),
        ("🇭🇰 港股", [_market_item_html("恒生指数", quotes["hsi"].get("price"), quotes["hsi"].get("change_pct"), _quote_meta(quotes["hsi"], "HK")), _market_item_html("恒生科技", quotes["hstech"].get("price"), quotes["hstech"].get("change_pct"), _quote_meta(quotes["hstech"], "HK"))], "two"),
        ("🇨🇳 A股", [_market_item_html("上证指数", quotes["sh"].get("price"), quotes["sh"].get("change_pct"), _quote_meta(quotes["sh"], "CN")), _market_item_html("深证成指", quotes["sz"].get("price"), quotes["sz"].get("change_pct"), _quote_meta(quotes["sz"], "CN")), _market_item_html("沪深300", quotes["csi300"].get("price"), quotes["csi300"].get("change_pct"), _quote_meta(quotes["csi300"], "CN"))], "three"),
    ]
...
def _render_quote_block(item):
    ...
    if item.get("market") == "US":
        ...
        if row.get("market_state") in ("POST", "POSTPOST", "CLOSED") and pp is not None:
            ...
            after = f'<div class="search-after">最近盘后：<strong>{pp:,.2f}</strong> <span>{"--" if pc is None else f"{pc:+.2f}%"}</span></div>'
    source = row.get("data_source") or row.get("quote_source") or ""
    delay = row.get("delayed_by")
    source_text = source
    if delay not in (None, 0, "0") and source == "Yahoo Finance":
        source_text = f"{source} · 延迟{delay}分"
    elif source:
        source_text = source
...
def _add_confirmed(key, item):
    ...
    if isinstance(confirmed, dict):
        confirmed = [confirmed]
    elif not isinstance(confirmed, list):
        confirmed = []
    if not any(x.get("symbol") == item.get("symbol") for x in confirmed if isinstance(x, dict)):
...
def filter_range(data, date_range):
    return data[data["observation_date"] >= get_start_date(date_range)].copy()
...
def render_core_charts():
    ...
    if compact_mode:
        cols = st.columns(3, gap="small")
        configs = [
            (cols[0], '<div class="compact-title">🏦 1. Fed Policy Rate</div>', '<div class="compact-description">IORB / ON RRP / EFFR / SOFR</div>', "compact_corridor_range", build_fig1, [("IORB", "https://fred.stlouisfed.org/series/IORB"), ("ON RRP", "https://fred.stlouisfed.org/series/RRPONTSYAWARD"), ("EFFR", "https://fred.stlouisfed.org/series/EFFR"), ("SOFR", "https://fred.stlouisfed.org/series/SOFR")], 0),
            (cols[1], '<div class="compact-title">2. 10Y Yield Structure</div>', '<div class="compact-description">10Y Nominal / Real / Breakeven</div>', "compact_yield10_range", build_fig2, [("DGS10", "https://fred.stlouisfed.org/series/DGS10"), ("DFII10", "https://fred.stlouisfed.org/series/DFII10"), ("T10YIE", "https://fred.stlouisfed.org/series/T10YIE")], 1),
            (cols[2], '<div class="compact-title">3. Treasury Yield</div>', '<div class="compact-description">3M / 2Y / 10Y / Curve Spread</div>', "compact_treasury_range", build_fig3, [("DGS3MO", "https://fred.stlouisfed.org/series/DGS3MO"), ("DGS2", "https://fred.stlouisfed.org/series/DGS2"), ("DGS10", "https://fred.stlouisfed.org/series/DGS10"), ("T10Y2Y", "https://fred.stlouisfed.org/series/T10Y2Y"), ("T10Y3M", "https://fred.stlouisfed.org/series/T10Y3M")], 2),
        ]
