...
def update_app() -> None:
    ...
    # Shared convention: series names carry (R1/R2/R3); right axes only show
    # compact ticks. Keep long semantic labels in subtitles / methodology.
    text = rep(text, '    base_right = 76 if has_secondary else 20', '    base_right = 60 if has_secondary else 20', 'shared right margin')

    # Chart 2 · 10Y structure.
    ...
    text = rep(
        text,
        'yaxis2=dict(title="Real / Breakeven (%)", overlaying="y", side="right", anchor="free", position=1.0, showgrid=False, zeroline=False, fixedrange=True, automargin=True, tickfont=dict(size=9))',
        'yaxis2=dict(title="", overlaying="y", side="right", anchor="free", position=0.99, showgrid=False, zeroline=False, fixedrange=True, automargin=False, tickfont=dict(size=9), ticks="outside", ticklen=3)',
        'chart2 compact right axis',
    )

    # Chart 3 · curve spreads share R1.
    text = rep(text, '10Y−2Y (R)', '10Y−2Y (R1)', 'chart3 10y2y labels', 4)
    text = rep(text, '10Y−3M (R)', '10Y−3M (R1)', 'chart3 10y3m labels', 4)
    text = rep(
        text,
        'yaxis2=dict(title="Spread (%)", overlaying="y", side="right", anchor="free", position=1.0, showgrid=False, zeroline=True, zerolinecolor="#9ca3af", fixedrange=True, automargin=True, tickfont=dict(size=9))',
        'yaxis2=dict(title="", overlaying="y", side="right", anchor="free", position=0.99, showgrid=False, zeroline=True, zerolinecolor="#9ca3af", fixedrange=True, automargin=False, tickfont=dict(size=9), ticks="outside", ticklen=3)',
        'chart3 compact right axis',
    )

    # Chart 4 · US liquidity, two compact right axes.
    ...
    text = rep(text, 'yaxis2=dict(overlaying="y", side="right", anchor="free", position=0.86),\n        yaxis3=dict(overlaying="y", side="right", anchor="free", position=0.97),', 'yaxis2=dict(overlaying="y", side="right", anchor="free", position=0.925),\n        yaxis3=dict(overlaying="y", side="right", anchor="free", position=0.99),', 'chart4 axis positions', 1)
    ...
    # Chart 9 · legend already carries R1/R2; only compact the axes.
    text = rep(text, 'margin=dict(l=62, r=144, t=68, b=36, pad=2)', 'margin=dict(l=62, r=92, t=68, b=36, pad=2)', 'chart9 margin')
    text = rep(text, 'xaxis=dict(domain=[0.0, 0.84])', 'xaxis=dict(domain=[0.0, 0.91])', 'chart9 domain', 1)
    ...
    # The predeclared positions for chart 9 are the second occurrence of this pair.
    text = rep(text, 'yaxis2=dict(overlaying="y", side="right", anchor="free", position=0.86),\n        yaxis3=dict(overlaying="y", side="right", anchor="free", position=0.97),', 'yaxis2=dict(overlaying="y", side="right", anchor="free", position=0.925),\n        yaxis3=dict(overlaying="y", side="right", anchor="free", position=0.99),', 'chart9 axis positions', 1)

    # Chart 10 · precious metals follows the same notation and spacing as crypto.
    ...
    text = rep(text, 'xaxis=dict(domain=[0.0, 0.84])', 'xaxis=dict(domain=[0.0, 0.91])', 'metals rebased domain', 1)
    ...
    # Visible subtitles and methodology use R1 consistently instead of generic R.
    text = rep(text, '10Y Nominal / 10Y Real (R) / 10Y Breakeven (R)', '10Y Nominal / 10Y Real (R1) / 10Y Breakeven (R1)', 'chart2 subtitle')
    text = rep(text, '3M / 2Y / 10Y / 10Y−2Y (R) / 10Y−3M (R)', '3M / 2Y / 10Y / 10Y−2Y (R1) / 10Y−3M (R1)', 'chart3 subtitle')
...
def update_hk() -> None:
    ...
    # Chart 5 / 8 rebased legends use R1; raw already has explicit R1/R2.
    ...
    # Chart 5 raw compact axes.
    text = rep(text, 'margin=dict(l=62, r=142, t=96, b=40, pad=2)', 'margin=dict(l=62, r=92, t=96, b=40, pad=2)', 'hk raw margins', 2)
    text = rep(text, 'xaxis=dict(domain=[0.0, 0.84])', 'xaxis=dict(domain=[0.0, 0.91])', 'hk raw domains', 2)
    text = rep(text, 'yaxis2=dict(overlaying="y", side="right", anchor="free", position=0.86, title="R1 · HKD Price", showgrid=False, fixedrange=True, tickfont=dict(size=10))', 'yaxis2=dict(overlaying="y", side="right", anchor="free", position=0.925, title="", showgrid=False, fixedrange=True, tickfont=dict(size=9), ticks="outside", ticklen=3)', 'hk raw r1 axes', 2)
    text = rep(text, 'yaxis3=dict(overlaying="y", side="right", anchor="free", position=0.97, title="R2 · Index Level", showgrid=False, fixedrange=True, tickfont=dict(size=10))', 'yaxis3=dict(overlaying="y", side="right", anchor="free", position=0.99, title="", showgrid=False, fixedrange=True, tickfont=dict(size=9), ticks="outside", ticklen=3)', 'hk raw r2 axes', 2)

    # Chart 5 / 8 rebased right axis: one small tick column only.
    text = rep(text, 'margin=dict(l=62, r=94, t=96, b=40, pad=2)', 'margin=dict(l=62, r=64, t=96, b=40, pad=2)', 'hk rebased margins', 2)
    text = rep(text, 'xaxis=dict(domain=[0.0, 0.91])', 'xaxis=dict(domain=[0.0, 0.95])', 'hk rebased domains', 2)
    text = rep(text, 'yaxis2=dict(title="Market · Rebased 100", showgrid=False, fixedrange=True)', 'yaxis2=dict(title="", overlaying="y", side="right", anchor="free", position=0.99, showgrid=False, fixedrange=True, tickfont=dict(size=9), ticks="outside", ticklen=3)', 'hk rebased axis', 2)

    # Remove redundant right-axis titles from update_yaxes calls.
    text = rep(text, 'title_text="R1 · HKD Price" if raw_market else "Market · Rebased 100", secondary_y=True,', 'title_text="", secondary_y=True,', 'hk market secondary titles', 2)
    ...
    # Chart 7 · spread is R1.
    text = rep(text, '"O/N−3M Spread (R)"', '"O/N−3M Spread (R1)"', 'hk chart7 spread')
    text = rep(text, 'title_text="Spread (bp)", secondary_y=True,\n        showgrid=False, zeroline=True, zerolinecolor="#cbd5e1", fixedrange=True,', 'title_text="", secondary_y=True,\n        showgrid=False, zeroline=True, zerolinecolor="#cbd5e1", fixedrange=True, tickfont=dict(size=9), ticks="outside", ticklen=3,', 'hk chart7 axis')
...
def main() -> None:
    update_app()
    update_hk()
    print("right-axis notation unified")
